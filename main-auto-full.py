...
def LP(train_pos, train_neg, test_pos, test_neg):
    ...
    for num_seed in [400,500,600,700,800,900]:
        for h_init in [0.05, 0.025, 0.0125]:
            train_graph_list = get_subgraph_set(train, net, hop, int(args.max_nodes_per_hop))
            ...
            if(auc > best_auc):
                best_auc = auc
                best_num_seed = num_seed
                best_h_init = h_init
    
    h_init = best_h_init
    num_seed = best_num_seed
    ...
    count = 0
    for c in [0.4,0.41,0.42,0.43,0.44,0.45,
